...
use Sakura\Branches\Exception\NotFoundException;
use Sakura\Sakura;
...
class ServerCommand extends Command {
    ...
    public function configure() {
        $this->setName('server')
            ...
            ->addOption('port', 'p', InputOption::VALUE_OPTIONAL, 'Port of the server', '8080')
            ->addOption('address', 'a', InputOption::VALUE_OPTIONAL, 'Server address', '127.0.0.1')
    ...
    public function mimeTypeForExtension($extension) {
    ...
    public function execute(InputInterface $input, OutputInterface $output) {
        ...
        $http->on('request', function (Request $request, Response $response) use ($directory, $self, $contentDirectory, $templatesDirectory) {
            try {
                $status       = 200;
                $headers      = array();
                $data         = null;
                $self->sakura = new Sakura($directory, $contentDirectory, null, $templatesDirectory);
                ...
                if($validPath) {
                    $filePath = $self->sakura->pathAt($path);

                    if(is_file($filePath) and is_readable($filePath)) {
                        $sendFile = true;
                        $path     = $filePath;
                    } else {
                        $filePath = $self->sakura->getContentDirectory($path);
                ...
                if($sendFile) {
                    $headers = array(
                        "Content-Type" => $self->mimeTypeForExtension(pathinfo($path, PATHINFO_EXTENSION)),
                    );
                    $data    = file_get_contents($path);
                } else {
                    try {
                        $page = $self->sakura->getBranches()->get($url);
                        $data = $self->sakura->renderPageAtUrl($page);
        ...
        $output->writeln("<info>Running at $address:$port...</info>");
