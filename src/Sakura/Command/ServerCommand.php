...
class ServerCommand extends Command {
    ...
    public function execute(InputInterface $input, OutputInterface $output) {
        ...
        $this->sakura = new Sakura($directory, $contentDirectory, null, $templatesDirectory);

        $http->on('request', function (Request $request, Response $response) use ($directory) {
            ...
            if($validPath) {
                $filePath = $this->sakura->pathAt($path);

                if(is_file($filePath) and is_readable($filePath)) {
                    $sendFile = true;
                    $path = $filePath;
                } else {
                    $filePath = $this->sakura->pathAt($this->sakura->getContentDirectory() . '/' . $path);
            ...
            if($sendFile) {
                $response->writeHead(200, array(
                    "Content-Type" => $this->mimeTypeForExtension(pathinfo($path, PATHINFO_EXTENSION)),
                ));
            ...
            try {
                $page = $this->sakura->getBranches()->get($url);
                $response->writeHead(200);
                $response->end($this->sakura->renderPageAtUrl($page));
